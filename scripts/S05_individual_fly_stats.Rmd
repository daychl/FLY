...
1. For pair-wise comparisons acorss genotypes within each treatment
``` {r}
library(emmeans) 

full_model <- lm(dev_time ~ genotype * treatment, data = individual_fly_data)
emm <- emmeans(full_model, ~ genotype | treatment) 
emm
...
2. For pair-wise comparison across treatments, within genotypes
``` {r}
library(emmeans) 

full_model <- lm(dev_time ~ genotype * treatment, data = individual_fly_data)
emm <- emmeans(full_model, ~ treatment | genotype) 
emm
